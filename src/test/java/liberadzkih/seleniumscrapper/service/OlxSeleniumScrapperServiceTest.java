...
class OlxSeleniumScrapperServiceTest {
    ...
    void should_find_at_least_one_item() {
        final var olxScrapperService = new OlxSeleniumScrapperService();
    ...
    void should_not_find_any_item() {
        final var items = new OlxSeleniumScrapperService().scrap(NO_ITEMS_URL);
        assertThat(items.size()).isEqualTo(0);
    }

    @Test
    void found_items_should_contain_necessary_data() {
        final var items = new OlxSeleniumScrapperService().scrap(MANY_ITEMS_URL);
        assertThat(items.size()).isGreaterThan(0);
